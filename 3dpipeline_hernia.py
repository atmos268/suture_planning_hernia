...
def project3d_to_2d(left_image, points):
    ...
    left_dist_coeffs = np.zeros((4,), np.float64)
...
if __name__ == "__main__":
    ...
    save_figs = True
    hernia_number = 2
    # left_img_path = 'hernia/3.webp'
    left_img_path = f'hernia/{hernia_number}.png'
    ...
    baseline_pth = "hernia/" + results_pth + "/baseline/"
    opt_pth = "hernia/" + results_pth + "/opt/"
    old_algo_pth = "hernia/" + results_pth + "/old_algo/"
    ...
    if mode == '2d' and experiment_mode == "physical":
        ...
        disp_path = f"hernia/{hernia_number}.npy"
        ...
        # np.save(f'hernia_results/point_cloud{hernia_number}.npy', point_cloud_data)
        ...
        np.save(f'hernia_results/rgb_cloud{hernia_number}.npy', rgb_cloud_data)
        ...
        border_pts_path = f'hernia/border_pts{hernia_number}.npy'
        ...
        np.save(f'hernia_results/line_pts_3d_{hernia_number}.npy', line_pts_3d)
        ...
        force_model_parameters = {'ellipse_ecc': 1/0.5, 'force_decay': 0.5/0.01, 'verbose': 0, 'ideal_closure_force': None, 'imparted_force': None}
        # force_model_parameters = {'ellipse_ecc': 1/0.5, 'force_decay': 0.5/0.12, 'verbose': 0, 'ideal_closure_force': None, 'imparted_force': None}
        left_image = cv2.imread(left_img_path, cv2.IMREAD_COLOR)
        ...
        np.save(f'hernia_results/insertion_pts{hernia_number}.npy', np.array(best_opt_insertion))
        np.save(f'hernia_results/extraction_pts{hernia_number}.npy', np.array(best_opt_extraction))
        ...
        suture_display_adjust.user_display_pnts(f"base{hernia_number}")
        ...
        # 1 - checking for crossings between sutures
        # reference for checking intersection between 2 segments: https://bryceboe.com/2006/10/23/line-segment-intersection-algorithm/
        def ccw(ax, ay, bx, by, cx, cy):
            return (cy - ay) * (bx - ax) > (by - ay) * (cx - ax)
        
        def two_sutures_intersect(ax, ay, bx, by, cx, cy, dx, dy):
            return ccw(ax, ay, cx, cy, dx, dy) != ccw(bx, by, cx, cy, dx, dy) and ccw(ax, ay, bx, by, cx, cy) != ccw(ax, ay, bx, by, dx, dy)
        
        def check_suture_cross(start, end):
            """
            Returns:
                bool: if the sutures cross each other
            """
            pairs = list(combinations(list(range(len(start))), 2))
            cross = []
            for i, j in pairs:
                ax, ay = start[i]
                bx, by = end[i]
                cx, cy = start[j]
                dx, dy = end[j]
                if two_sutures_intersect(ax, ay, bx, by, cx, cy, dx, dy):
                    return True
            return False
        
        # 2 - check if sutures are too close together
        def euc_dist(p1, p2):
            return np.sqrt((p1[0] - p2[0]) ** 2 + (p1[1] - p2[1]) ** 2)
        
        def check_suture_dist(pts, thresh):
            """
            Returns:
                bool: if suture points are too close together
            """
            for i in range(len(pts)-1):
                if euc_dist(pts[i], pts[i+1]) > thresh:
                    return True
            return False
        
        min_dist_thresh = 1000
        viz = True
        if use_metrics:
            viz = not check_suture_cross(insertion_pts, extraction_pts) and not check_suture_dist(insertion_pts, min_dist_thresh) and not check_suture_dist(extraction_pts, min_dist_thresh)
            print("METRICS:",check_suture_cross(insertion_pts, extraction_pts), check_suture_dist(insertion_pts, min_dist_thresh), check_suture_dist(extraction_pts, min_dist_thresh))
        suture_display_adjust_optim = SutureDisplayAdjust(insertion_pts, center_pts, extraction_pts, left_image, center_spline)
        suture_display_adjust_optim.user_display_pnts(f"opt{hernia_number}", viz=viz)
        ...
        if viz:
            ...
            print("Input image does not meet the constraints, and thus a suture plan could not be generated.")
